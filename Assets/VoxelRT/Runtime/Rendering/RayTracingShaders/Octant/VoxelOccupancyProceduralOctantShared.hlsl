#ifndef VOXEL_OCCUPANCY_PROCEDURAL_OCTANT_SHARED_INCLUDED
#define VOXEL_OCCUPANCY_PROCEDURAL_OCTANT_SHARED_INCLUDED
...
static const uint kVoxelChunkSmallOctantDimension = 2u;
static const uint kVoxelChunkSmallOctantGridDimension = kVoxelChunkDimension / kVoxelChunkSmallOctantDimension;
...
uint FlattenSmallOctantCoord(uint3 smallOctantCoord)
{
    return
        (smallOctantCoord.x & 1u) |
        ((smallOctantCoord.y & 1u) << 1u) |
        ((smallOctantCoord.z & 1u) << 2u) |
        (((smallOctantCoord.x >> 1u) & 1u) << 3u) |
        (((smallOctantCoord.y >> 1u) & 1u) << 4u) |
        (((smallOctantCoord.z >> 1u) & 1u) << 5u);
}

uint ComputeSmallOctantByteIndex(int3 smallOctantCoord)
{
    return FlattenSmallOctantCoord((uint3)smallOctantCoord);
}

uint ComputeVoxelBitIndexInSmallOctant(int3 voxelCoord)
{
    return
        ((uint)voxelCoord.x & 1u) |
        (((uint)voxelCoord.y & 1u) << 1u) |
        (((uint)voxelCoord.z & 1u) << 2u);
}

uint ReadSmallOctantOccupancyByte(uint chunkGlobalIndex, int3 smallOctantCoord)
{
    uint byteAddress =
        (chunkGlobalIndex * kVoxelChunkOccupancyByteCount) +
        ComputeSmallOctantByteIndex(smallOctantCoord);
    return ReadOccupancyByte(byteAddress);
}

struct OccupancyTraversalState
{
    int3 mediumOctantCoord;
    uint mediumWordLo;
    uint mediumWordHi;
    uint smallIndex;
    uint occupancyByte;
...
uint ComputeMediumOctantIndex(int3 mediumOctantCoord)
{
    return
        (uint)mediumOctantCoord.x |
        ((uint)mediumOctantCoord.y << 1u) |
        ((uint)mediumOctantCoord.z << 2u);
}

uint ComputeSmallOctantIndexInMedium(int3 voxelCoord)
{
    return
        (((uint)voxelCoord.x >> 1u) & 1u) |
        ((((uint)voxelCoord.y >> 1u) & 1u) << 1u) |
        ((((uint)voxelCoord.z >> 1u) & 1u) << 2u);
}

void ReadMediumOctantOccupancyWords(
    ...
    out uint wordHi)
{
    uint byteAddress =
        (chunkGlobalIndex * kVoxelChunkOccupancyByteCount) +
        (ComputeMediumOctantIndex(mediumOctantCoord) << 3u);
    wordLo = _VoxelOccupancyChunkBuffer.Load(byteAddress);
    wordHi = _VoxelOccupancyChunkBuffer.Load(byteAddress + 4u);
}

uint ExtractSmallOctantOccupancyByte(uint wordLo, uint wordHi, uint smallIndex)
{
    uint packedWord = smallIndex < 4u ? wordLo : wordHi;
    uint byteShift = (smallIndex & 3u) * 8u;
    return (packedWord >> byteShift) & 0xFFu;
}

void InitOccupancyTraversalState(
    uint chunkGlobalIndex,
    int3 voxelCoord,
    out OccupancyTraversalState state)
{
    state.mediumOctantCoord = int3(voxelCoord.x >> 2, voxelCoord.y >> 2, voxelCoord.z >> 2);
    state.smallIndex = ComputeSmallOctantIndexInMedium(voxelCoord);
    state.bitIndex = ComputeVoxelBitIndexInSmallOctant(voxelCoord);
    ReadMediumOctantOccupancyWords(
        chunkGlobalIndex,
        state.mediumOctantCoord,
        state.mediumWordLo,
        state.mediumWordHi);
    state.occupancyByte = ExtractSmallOctantOccupancyByte(
        state.mediumWordLo,
        state.mediumWordHi,
        state.smallIndex);
}

bool IsCurrentVoxelOccupied(OccupancyTraversalState state)
{
    return (state.occupancyByte & (1u << state.bitIndex)) != 0u;
}

uint ComputeForwardStepMask(int stepSign)
{
    return stepSign > 0 ? 1u : 0u;
}

uint ComputeAxisParity(uint packedIndex, int axis)
{
    return (packedIndex >> (uint)axis) & 1u;
}

void AdvanceOccupancyTraversalState(
    uint chunkGlobalIndex,
    int axis,
    int stepSign,
    inout OccupancyTraversalState state)
{
    uint axisBit = 1u << (uint)axis;
    uint forwardMask = ComputeForwardStepMask(stepSign);
    uint crossedSmallBoundary = 1u ^ (ComputeAxisParity(state.bitIndex, axis) ^ forwardMask);
    uint crossedMediumBoundary = crossedSmallBoundary &
        (1u ^ (ComputeAxisParity(state.smallIndex, axis) ^ forwardMask));

    state.bitIndex ^= axisBit;
    state.smallIndex ^= axisBit * crossedSmallBoundary;

    if (crossedMediumBoundary != 0u)
    {
        int3 axisMask = int3(axis == 0 ? 1 : 0, axis == 1 ? 1 : 0, axis == 2 ? 1 : 0);
        state.mediumOctantCoord += axisMask * stepSign;
        ReadMediumOctantOccupancyWords(
            chunkGlobalIndex,
            state.mediumOctantCoord,
            state.mediumWordLo,
    ...
    out bool traversalTerminated)
{
    ...
    if (any(state.voxel < 0) || any(state.voxel >= volumeSize))
    {
        return false;
    }

    OccupancyTraversalState occupancyState;
    InitOccupancyTraversalState(chunkGlobalIndex, state.voxel, occupancyState);
    ...
    for (int stepIndex = 0; stepIndex < rootStepCount; stepIndex++)
    {
        ...
        if (cellExitT > state.t && IsCurrentVoxelOccupied(occupancyState))
        {
        ...
        StepDda(state);
        if (state.t < state.tLimit && all(state.voxel >= 0) && all(state.voxel < volumeSize))
        {
            AdvanceOccupancyTraversalState(
                chunkGlobalIndex,
                state.lastAxis,
                state.lastStepSign,
                occupancyState);
