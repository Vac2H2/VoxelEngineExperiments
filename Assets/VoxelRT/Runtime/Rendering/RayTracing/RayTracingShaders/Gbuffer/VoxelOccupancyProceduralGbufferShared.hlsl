#ifndef VOXEL_OCCUPANCY_PROCEDURAL_GBUFFER_SHARED_INCLUDED
#define VOXEL_OCCUPANCY_PROCEDURAL_GBUFFER_SHARED_INCLUDED
...
static const uint kPaletteChunkByteCount = 1024u;
...
ByteAddressBuffer _VoxelPaletteChunkBuffer;
StructuredBuffer<uint> _VoxelPaletteChunkStartBuffer;
StructuredBuffer<uint> _VoxelSurfaceTypeTableBuffer;
...
ByteAddressBuffer _VoxelDataChunkBuffer;
...
{
    return (packedPaletteEntry >> 24u) & 0xFFu;
}
...
uint ReadPaletteEntry(uint paletteResidencyId, uint paletteEntryIndex)
{
    uint paletteChunkSlot = _VoxelPaletteChunkStartBuffer[paletteResidencyId];
    if (paletteChunkSlot == 0xffffffffu)
    {
        return 0u;
    }

    uint byteAddress = (paletteChunkSlot * kPaletteChunkByteCount) + (paletteEntryIndex * 4u);
    return _VoxelPaletteChunkBuffer.Load(byteAddress);
}

uint ReadSurfaceInfoEntry(uint surfaceType)
{
    return _VoxelSurfaceTypeTableBuffer[surfaceType & 0xFFu];
}
...
bool IsVoxelOccupied(uint chunkGlobalIndex, int3 voxelCoord)
{
    ...
    uint occupancyByte = ReadRawByte(_VoxelOccupancyChunkBuffer, byteAddress);
    return (occupancyByte & (1u << (uint)voxelCoord.x)) != 0u;
}

uint ReadPaletteEntryIndex(uint chunkGlobalIndex, uint localVoxelIndex)
{
    uint byteAddress = (chunkGlobalIndex * kVoxelChunkVoxelByteCount) + localVoxelIndex;
    return ReadRawByte(_VoxelDataChunkBuffer, byteAddress);
}
    ...
    AttributeData attributes)
{
    ...
    uint packedHitData = attributes.packedHitData;
    uint primitiveIndex = PrimitiveIndex();
    bool isValidChunk;
    uint chunkGlobalIndex = ResolveChunkGlobalIndex(primitiveIndex, isValidChunk);
    uint faceId = DecodeFaceIdFromHitData(packedHitData);
    uint localVoxelIndex = DecodeLocalVoxelIndex(packedHitData);
