Shader "Hidden/VoxelRT/Rendering/RtLightingTemporalDenoise"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D_float _VoxelRtVelocity;
            ...
            float4 frag(v2f_img input) : SV_Target
            {
                ...
                float currentDepth = tex2D(_VoxelRtDepth, uv).r;
                if (currentDepth <= 0.0)
                {
                    return float4(currentLighting, 1.0);
                }

                float2 velocity = tex2D(_VoxelRtVelocity, uv).rg;
                ...
                float3 currentNormal = DecodeLightingNormal(tex2D(_VoxelRtNormal, uv).xyz);
