...
`AO` preview reads the dedicated `_VoxelExperimentsAo` scalar RT, then treats that value
as ambient visibility and displays `white * visibility`, so open space stays
bright and nearby occlusion darkens the preview.

`SunLight` preview now reads the dedicated `_VoxelExperimentsSunLight` color RT directly.
That target already stores HDR RGB sunlight, so no scalar-to-color preview
conversion is needed.

`LocalLight` preview reads the dedicated `_VoxelExperimentsLocalLight` color RT directly.
...
`_VoxelExperimentsScalarPreview` color target and then blitted to the camera output.
...
- `_VoxelExperimentsScalarPreview`
