Shader "Hidden/VoxelExperiments/Rendering/RtLightingSpatialDenoise"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f_img input) : SV_Target
            {
                ...
                float centerDepth = tex2D(_VoxelExperimentsDepth, uv).r;
                if (centerDepth <= 0.0)
                {
                    return float4(centerLighting, 1.0);
                }

                float3 centerNormal = DecodeLightingNormal(tex2D(_VoxelExperimentsNormal, uv).xyz);
                ...
                for (int offsetY = -1; offsetY <= 1; offsetY++)
                {
                    ...
                    for (int offsetX = -1; offsetX <= 1; offsetX++)
                    {
                        ...
                        float sampleDepth = tex2D(_VoxelExperimentsDepth, sampleUv).r;
                        if (sampleDepth <= 0.0)
                        {
                            continue;
                        }

                        float3 sampleNormal = DecodeLightingNormal(tex2D(_VoxelExperimentsNormal, sampleUv).xyz);
