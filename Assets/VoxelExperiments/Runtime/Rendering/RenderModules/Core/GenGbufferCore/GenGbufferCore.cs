...
namespace VoxelExperiments.Runtime.Rendering.RenderModules.Core
{
    ...
    public sealed class GenGbufferCore
    {
            ...
            in RenderData renderData)
        {
            ...
            BindOutputTexture(commandBuffer, VoxelExperimentsGbufferTexture.Albedo);
            BindOutputTexture(commandBuffer, VoxelExperimentsGbufferTexture.Normal);
            BindOutputTexture(commandBuffer, VoxelExperimentsGbufferTexture.Depth);
            BindOutputTexture(commandBuffer, VoxelExperimentsGbufferTexture.SurfaceInfo);
            BindOutputTexture(commandBuffer, VoxelExperimentsGbufferTexture.Velocity);
            ...
            ExposeGlobalTexture(commandBuffer, VoxelExperimentsGbufferTexture.Albedo);
            ExposeGlobalTexture(commandBuffer, VoxelExperimentsGbufferTexture.Normal);
            ExposeGlobalTexture(commandBuffer, VoxelExperimentsGbufferTexture.Depth);
            ExposeGlobalTexture(commandBuffer, VoxelExperimentsGbufferTexture.SurfaceInfo);
            ExposeGlobalTexture(commandBuffer, VoxelExperimentsGbufferTexture.Velocity);
        ...
        public void ReleaseTemporaryTargets(CommandBuffer commandBuffer)
        {
            ...
            commandBuffer.ReleaseTemporaryRT(VoxelExperimentsGbufferIds.AlbedoTextureId);
            commandBuffer.ReleaseTemporaryRT(VoxelExperimentsGbufferIds.NormalTextureId);
            commandBuffer.ReleaseTemporaryRT(VoxelExperimentsGbufferIds.DepthTextureId);
            commandBuffer.ReleaseTemporaryRT(VoxelExperimentsGbufferIds.SurfaceInfoTextureId);
            commandBuffer.ReleaseTemporaryRT(VoxelExperimentsGbufferIds.VelocityTextureId);
        }

        private void AllocateTemporaryTargets(CommandBuffer commandBuffer, int width, int height)
        {
            commandBuffer.GetTemporaryRT(
                VoxelExperimentsGbufferIds.AlbedoTextureId,
                CreateTextureDescriptor(width, height, GraphicsFormat.R8G8B8A8_UNorm),
                FilterMode.Point);
            commandBuffer.GetTemporaryRT(
                VoxelExperimentsGbufferIds.NormalTextureId,
                CreateTextureDescriptor(width, height, GraphicsFormat.R8G8B8A8_UNorm),
                FilterMode.Point);
            commandBuffer.GetTemporaryRT(
                VoxelExperimentsGbufferIds.DepthTextureId,
                CreateTextureDescriptor(width, height, GraphicsFormat.R16_SFloat),
                FilterMode.Point);
            commandBuffer.GetTemporaryRT(
                VoxelExperimentsGbufferIds.SurfaceInfoTextureId,
                CreateTextureDescriptor(width, height, GraphicsFormat.R8G8B8A8_UNorm),
                FilterMode.Point);
            commandBuffer.GetTemporaryRT(
                VoxelExperimentsGbufferIds.VelocityTextureId,
                CreateTextureDescriptor(width, height, GraphicsFormat.R16G16_SFloat),
                FilterMode.Point);
        }

        private void BindOutputTexture(CommandBuffer commandBuffer, VoxelExperimentsGbufferTexture texture)
        {
        ...
        private static void ExposeGlobalTexture(CommandBuffer commandBuffer, VoxelExperimentsGbufferTexture texture)
        {
            int textureId = VoxelExperimentsGbufferIds.GetTextureId(texture);
