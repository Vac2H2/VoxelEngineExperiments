...
Texture2D<float4> _VoxelExperimentsNormal;
Texture2D<float> _VoxelExperimentsDepth;
...
{
    float linearViewDepth = _VoxelExperimentsDepth.Load(int3(pixel, 0));
    ...
    normalWS = DecodeLightingNormal(_VoxelExperimentsNormal.Load(int3(pixel, 0)).xyz);
