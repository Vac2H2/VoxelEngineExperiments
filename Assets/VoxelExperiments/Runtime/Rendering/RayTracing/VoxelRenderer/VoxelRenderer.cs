...
using VoxelRuntimeServices = VoxelExperiments.Runtime.Rendering.VoxelRuntime.VoxelRuntime;

namespace VoxelExperiments.Runtime.Rendering.VoxelRenderer
{
    [ExecuteAlways]
    ...
    {
#if UNITY_EDITOR
        private static bool s_isExitingPlayMode;

        [InitializeOnLoadMethod]
        private static void RegisterPlayModeStateCallbacks()
        {
            EditorApplication.playModeStateChanged -= HandlePlayModeStateChanged;
            EditorApplication.playModeStateChanged += HandlePlayModeStateChanged;
            s_isExitingPlayMode = false;
        }
        ...
        private const uint OpaqueInstanceMask = 1u << 0;
        private const uint TransparentInstanceMask = 1u << 1;
        ...
        private void LateUpdate()
        {
            ...
            {
                // Transform-driven RTAS updates are intentionally disabled.
        ...
        {
            if (ShouldSkipReleaseBecauseWorldIsStopping())
            {
        ...
        private void ProcessPendingChanges()
        {
            ...
            // Large voxel prefabs can exhaust Unity's editor-side D3D12 upload budget
            // when they auto-register into the RTAS during edit mode. Defer all GPU
            // residency and RTAS registration until play mode starts.
            if (!Application.IsPlaying(gameObject))
            {
                // When the play-world is stopping, releasing each renderer one-by-one
                // causes residency services to compact and re-upload large buffers.
                // Let the runtime bootstrap dispose the whole resource system instead.
                ForgetRuntimeBinding();
                return;
            }
            ...
            if (bootstrap.IsGracefulShutdownRequested)
            {
                ForgetRuntimeBinding();
                return;
            }
        ...
        private bool ShouldSkipReleaseBecauseWorldIsStopping()
        {
            if (!Application.IsPlaying(gameObject))
            {
                return true;
        ...
        internal void PrepareForGracefulShutdown(VoxelRuntimeBootstrap bootstrap)
        {
            if (bootstrap == null)
            {
            ...
            if (_rtasHandle != InvalidHandle)
            {
                bootstrap.Runtime.RayTracingScene.RemoveInstance(_rtasHandle);
            }
            ...
            Exception exception)
        {
            ...
            builder.AppendLine(
                $"Renderer='{name}', Active={isActiveAndEnabled}, Layer={gameObject.layer}, " +
                $"InstanceMask={ResolveInstanceMask()}");
            builder.AppendLine(
        ...
        private uint ResolveInstanceMask()
        {
