...
namespace VoxelEngine.Render.Cores
{
    public static class VoxelRtaoIds
    {
        ...
        public static readonly int MaxDistanceId = Shader.PropertyToID("_VoxelEngineRtaoMaxDistance");
    ...
    public sealed class RtaoCore
    {
        ...
        private static readonly int RtaoMaxDistanceParamId = Shader.PropertyToID("_RtaoMaxDistance");
        private static readonly int RtaoNormalBiasParamId = Shader.PropertyToID("_RtaoNormalBias");
        private static readonly int RtaoRaysPerPixelParamId = Shader.PropertyToID("_RtaoRaysPerPixel");
        ...
        [SerializeField] private float _maxDistance = DefaultMaxDistance;
        [SerializeField] private float _normalBias = DefaultNormalBias;
            ...
            VoxelEngineRenderBackend renderBackend)
        {
            ...
            try
            {
                ...
                commandBuffer.SetRayTracingTextureParam(_rayTracingShader, VoxelGbufferIds.NormalTextureId, VoxelGbufferIds.NormalTarget);
                ...
                commandBuffer.SetRayTracingTextureParam(_rayTracingShader, VoxelRtaoIds.OutputTextureId, VoxelRtaoIds.OutputTarget);
                commandBuffer.DispatchRays(
                    ...
                    1u,
                    camera);
                commandBuffer.SetGlobalTexture(VoxelRtaoIds.OutputTextureId, VoxelRtaoIds.OutputTarget);
                commandBuffer.SetGlobalFloat(VoxelRtaoIds.MaxDistanceId, ResolveMaxDistance());
                return true;
            }
            catch
            {
                commandBuffer.ReleaseTemporaryRT(VoxelRtaoIds.OutputTextureId);
                throw;
            }
        }
        ...
        {
            return _resolutionMode == RtaoResolutionMode.Half ? 2 : 1;
        }
        ...
        {
            return _maxDistance <= 0.0f ? DefaultMaxDistance : _maxDistance;
        }

        private float ResolveNormalBias()
        {
            return _normalBias < 0.0f ? 0.0f : (_normalBias == 0.0f ? DefaultNormalBias : _normalBias);
        }

        private int ResolveRaysPerPixel()
        {
            return Mathf.Clamp(_raysPerPixel, MinRaysPerPixel, MaxRaysPerPixel);
        }
